...
📖 [English](./README.md) | 📐 [Architecture](./docs/architecture.md) | 📐 [Architecture (한국어)](./docs/architecture-ko.md)
...
# ⚠️ CLI 플래그는 쉘 히스토리와 프로세스 목록에 노출될 수 있습니다.
# 프로덕션 환경에서는 환경변수 또는 대화형 프롬프트를 사용하세요.

# 현재 설정 표시 (키는 마스킹)
dd-cli auth configure show
```

키는 `~/.config/dd-cli/config.json`에 저장 (`XDG_CONFIG_HOME` 존중, 파일 권한 `0600`).

환경변수로도 API Key 설정 가능:
...
**인증 우선순위:** 환경변수 > 설정 파일 > OAuth 토큰 > 에러
...
- [@datadog/datadog-api-client](https://www.npmjs.com/package/@datadog/datadog-api-client) — 공식 Datadog SDK
