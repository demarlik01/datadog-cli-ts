...
│   │   ├── config.ts          # 인증/설정 초기화 (async, OAuth + API Key 지원)
│   │   ├── logs.ts            # LogsApi 래퍼
...
/**
 * SDK 설정을 생성한다.
 * OAuth 토큰 자동 갱신이 필요하므로 비동기 함수.
 *
 * 우선순위: 환경변수(API Key) > OAuth 토큰 > 에러
 */
...
**주요 변경사항:** `createConfig()`가 이제 `async` 함수. OAuth 토큰 자동 갱신이 비동기 호출을 필요로 하기 때문. 모든 client 함수에서 `await createConfig()`를 사용.
...
2. OAuth 토큰 (~/.config/dd-cli/tokens_{site}.json) → OAuth2 인증 (자동 갱신)
3. 둘 다 없음 → 에러 메시지와 안내
```
...
| 파일 | 경로 | 내용 |
|------|------|------|
| 토큰 | `~/.config/dd-cli/tokens_{site}.json` | `access_token`, `refresh_token`, `expires_in`, `issued_at`, `scope` |
| 클라이언트 | `~/.config/dd-cli/client_{site}.json` | `client_id`, `client_name`, `redirect_uris`, `registered_at` |
...
- `"1h"` → `new Date(now - 1시간).toISOString()`
- `"24h"` → `new Date(now - 24시간).toISOString()`
- `"7d"` → `new Date(now - 7일).toISOString()`
